...
"""
num_list = [ 2 , 8 , 10 , 15  , 7 , 13 , 11 , 4  , 4 , 8 ]
target = 15
...
#Approach 2 : Using Hashmap -> Not allowing pairs that alrady exist in the list . eg: (7,4) , (4,7) not allowed
def two_sum_two(num_list,target):
    num_list.sort()
    pairs_found = {}
    for i in range(len(num_list)-1):
        the_other_number = target - num_list[i]
        if the_other_number in pairs_found:
            continue
        else:
